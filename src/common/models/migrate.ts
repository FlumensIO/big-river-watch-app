...
import { db, mainStore, recordsStore } from 'common/models/store';
...
export default async () => {
  ...
  try {
    ...
    if (hasModels) {
      await db.query({
        sql: `INSERT OR IGNORE INTO records (id, cid, data, created_at, updated_at, synced_at)
        SELECT
              json(value) ->> "$.id",
              key,
              json(value),
      ...
      console.log('SQLite migrate: records migrated');
    }
  } catch (error) {
    console.error(error);
    showError(error);
